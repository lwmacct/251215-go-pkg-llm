...
func TestStreamParser_handleToolCall_IncrementalUpdates(t *testing.T) {
	...
	assert.Equal(t, `{"key":"value"}`, buf.argsBuf)
}
...
func TestStreamParser_MultipleToolsOutOfOrder(t *testing.T) {
	...
	tool0 := msg.ContentBlocks[0].(*llm.ToolCall)
	tool1 := msg.ContentBlocks[1].(*llm.ToolCall)
	tool2 := msg.ContentBlocks[2].(*llm.ToolCall)
...
func TestStreamParser_Parse_ReasoningWithToolCalls(t *testing.T) {
	...
	textBlock := result.Message.ContentBlocks[0].(*llm.TextBlock)
	assert.Equal(t, "Let me search.", textBlock.Text)

	toolBlock := result.Message.ContentBlocks[1].(*llm.ToolCall)
