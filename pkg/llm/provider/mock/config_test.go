...
func TestScenario_Template(t *testing.T) {
	t.Setenv("BOT_NAME", "测试机器人")
	t.Setenv("VERSION", "v1.0")
...
func TestScenario_MultipleTools(t *testing.T) {
	...
	toolCall1, ok := resp.Message.ContentBlocks[1].(*llm.ToolCall)
	require.True(t, ok)
	toolCall2, ok := resp.Message.ContentBlocks[2].(*llm.ToolCall)
	require.True(t, ok)
	assert.Equal(t, "tool1", toolCall1.Name)
	assert.Equal(t, "tool2", toolCall2.Name)
}
...
func TestGetLastInput(t *testing.T) {
	...
	// 无调用时返回空
	assert.Empty(t, client.GetLastInput())
